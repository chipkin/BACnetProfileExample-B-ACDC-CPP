...
**B-ACDC (Access Control Door Controller)** device profile in C++ using the
...
**WriteProperty** requests, is discoverable via **Who-Is / I-Am**, and exposes
one **commandable Access Door** that a client locks and unlocks.

**[Download a prebuilt binary](https://github.com/chipkin/BACnetProfileExample-B-ACDC-CPP/releases)**
...
Device 389011  "Rainbow"   (Vendor 389 - Chipkin Automation Systems)
    ...
    ├── Access Door 1        "Cobalt"      Present_Value  lock    (commandable - WriteProperty locks/unlocks)
    └── Network Port 1       "Vermilion"   the BACnet/IP port     (required on every device)
```

The three sensor objects are read-only, exactly as in the
[B-SS example](https://github.com/chipkin/BACnetProfileExample-B-SS-CPP). The
**Access Door** is the one object that makes this a B-ACDC: its `Present_Value`
is not stored directly, but resolved by the stack from a 16-slot
**`Priority_Array`** plus a **`Relinquish_Default`** of `lock` - a
`WriteProperty(Present_Value, value, priority)` sets a slot, `NULL` relinquishes
it, and the stack reports the highest-priority non-null slot (or
`Relinquish_Default`) as the effective value. A fire-alarm system can command
the door at a high priority and a badge reader at a low one, and BACnet decides
who wins - no application logic required. `Present_Value` is a
**BACnetDoorValue**: `0` = lock, `1` = unlock, `2` = pulseUnlock,
`3` = extendedPulseUnlock.
...
| DS-ACAD-B | Data Sharing - Access Control Access Door - B (the profile's defining BIBB) | ✅ |
...
| WriteProperty | Accepts a door command into the Priority_Array (DS-WP-B). |
...
[prebuilt release binary](https://github.com/chipkin/BACnetProfileExample-B-ACDC-CPP/releases).
...
```bash
# Linux / macOS
./build/BACnetExampleBACDC

# Windows
.\build\Release\BACnetExampleBACDC.exe
```

Expected output:

```
BACnet B-ACDC (Access Control Door Controller) Example - C++ v1.1.0
...
| `--deviceID <n>` | `389011` | The device's BACnet instance number (BACnet requires this to be configurable). |
...
3. **Read the Access Door** - ReadProperty `Access Door 1` -> `Present_Value`
   ...
   `Relinquish_Default`).
4. **Command it** - WriteProperty `Access Door 1` `Present_Value` = `1` (unlock)
   at **priority 8** -> read it back: `unlock`. `Lock_Status` now reads
   `unlocked` and `Secured_Status` reads `unsecured`.
5. **Confirm command arbitration** - WriteProperty `Present_Value` = `0` (lock)
   at **priority 1** -> the door reads `lock`: priority 1 outranks the
   priority-8 unlock, even though the unlock came later.
6. **Relinquish** - WriteProperty `Present_Value` = `NULL` at priority 1 -> the
   door returns to `unlock` (slot 8 is still commanded). WriteProperty `NULL` at
   priority 8 too -> every slot is now null, so the door falls back to
   `Relinquish_Default` = `lock`.
7. **Confirm validation** - WriteProperty `Present_Value` = `9` -> rejected with
   `value-out-of-range`.
...
[v1.1.0](https://github.com/chipkin/BACnetProfileExample-B-ACDC-CPP/releases/tag/v1.1.0)
(`metrics-windows.json` / `metrics-linux.json`):

<!-- METRICS -->
| Platform | Binary | Size | Start-up to `ready` | Stack commit |
|---|---|---|---|---|
| Windows x64 (windows-2022) | `BACnetExampleBACDC.exe` | 3,249,152 bytes (~3.1 MiB) | 74 ms | `abd4cee1` |
| Linux x64 (ubuntu-latest) | `BACnetExampleBACDC` | 43,336 bytes (~42 KiB) | 12 ms | `abd4cee1` |

v1.1.0 was published from a prebuilt stack library (`CAS_BACNET_STACK_LINK=STATIC`)
